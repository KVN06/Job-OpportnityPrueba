...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        <h1 class="text-2xl font-bold text-gray-800 mb-6">Editar Perfil de Desempleado</h1>

        <form action="{{ route('unemployed.update', $unemployed) }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-sm p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="profession" id="profession" value="{{ old('profession', $unemployed->profession) }}" 
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="experience_years" id="experience_years" value="{{ old('experience_years', $unemployed->experience_years) }}" 
                ...
                <div class="mb-4">
                    ...
                    <select name="experience_level" id="experience_level" 
                        ...
                        <option value="junior" {{ old('experience_level', $unemployed->experience_level) == 'junior' ? 'selected' : '' }}>Junior</option>
                        <option value="medio" {{ old('experience_level', $unemployed->experience_level) == 'medio' ? 'selected' : '' }}>Medio</option>
                        <option value="senior" {{ old('experience_level', $unemployed->experience_level) == 'senior' ? 'selected' : '' }}>Senior</option>
                        <option value="lead" {{ old('experience_level', $unemployed->experience_level) == 'lead' ? 'selected' : '' }}>Lead</option>
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="location" id="location" value="{{ old('location', $unemployed->location) }}" 
                           class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                    @error('location')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
            ...
            <div class="mb-6">
                ...
                <div class="mb-4">
                    ...
                    <textarea name="skills" id="skills" rows="3" 
                              ...
                              placeholder="Ej: PHP, Laravel, JavaScript, MySQL...">{{ old('skills', $unemployed->skills) }}</textarea>
                    @error('skills')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="experience" id="experience" rows="4" 
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" 
                              placeholder="Describe tu experiencia laboral...">{{ old('experience', $unemployed->experience) }}</textarea>
                    @error('experience')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="mb-4">
                    ...
                    <textarea name="education" id="education" rows="3" 
                              class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" 
                              placeholder="Describe tu formación académica...">{{ old('education', $unemployed->education) }}</textarea>
                    @error('education')
                        <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                    @enderror
                </div>
            ...
            <div class="mb-6">
                ...
                @if($unemployed->cv)
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700">CV Actual</label>
                    <div class="mt-1 flex items-center space-x-2">
                        <a href="{{ asset('storage/' . $unemployed->cv) }}" target="_blank"
                           class="text-blue-600 hover:text-blue-800 text-sm">Ver CV actual</a>
                        <span class="text-gray-500 text-sm">|</span>
                        <span class="text-gray-500 text-sm">Subir nuevo CV para reemplazar</span>
                    </div>
                </div>
                @endif
                ...
                <div class="mb-4">
                    <label for="cv" class="block text-sm font-medium text-gray-700">
                        {{ $unemployed->cv ? 'Nuevo Curriculum Vitae (PDF)' : 'Curriculum Vitae (PDF)' }}
                    </label>
                ...
                <div class="mb-4">
                    <div class="flex items-center">
                        <input type="checkbox" name="remote_work" id="remote_work" value="1" 
                               {{ old('remote_work', $unemployed->remote_work) ? 'checked' : '' }}
                ...
                <div class="mb-4">
                    ...
                    <input type="number" name="expected_salary" id="expected_salary" value="{{ old('expected_salary', $unemployed->expected_salary) }}" 
...
<div id="deleteModal" class="fixed inset-0 bg-gray-600 bg-opacity-50 hidden z-50">
    <div class="flex items-center justify-center min-h-screen">
        <div class="bg-white rounded-lg p-6 max-w-sm mx-auto">
            ...
            <p class="text-sm text-gray-500 mb-4">¿Estás seguro de que quieres eliminar tu perfil? Esta acción no se puede deshacer.</p>
            <div class="flex justify-end space-x-2">
                ...
                <form action="{{ route('unemployed.destroy', $unemployed) }}" method="POST" class="inline">
                    @csrf
                    @method('DELETE')
                    <button type="submit" 
