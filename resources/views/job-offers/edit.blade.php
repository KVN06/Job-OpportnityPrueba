...
<div class="container mx-auto px-4 py-8">
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('job-offers.update', $jobOffer) }}" method="POST" class="bg-white rounded-lg shadow-sm p-6">
            ...
            <!-- Campo para editar el título de la oferta -->
            <div class="mb-4">
                <label for="title" class="block text-sm font-medium text-gray-700">Título</label>
                <input type="text" name="title" id="title" value="{{ old('title', $jobOffer->title) }}" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                @error('title')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p> <!-- Mensaje de error para título -->
                @enderror
            </div>

            <!-- Campo para editar la descripción de la oferta -->
            <div class="mb-4">
                ...
                @error('description')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p> <!-- Mensaje de error para descripción -->
                @enderror
            </div>

            <!-- Campo para editar el salario ofrecido -->
            <div class="mb-4">
                ...
                @error('salary')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p> <!-- Mensaje de error para salario -->
                @enderror
            </div>

            <!-- Campo para editar la ubicación de la oferta -->
            <div class="mb-4">
                <label for="location" class="block text-sm font-medium text-gray-700">Ubicación</label>
                <input type="text" name="location" id="location" value="{{ old('location', $jobOffer->location) }}" 
                       class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-blue-500 focus:ring-blue-500" required>
                @error('location')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p> <!-- Mensaje de error para ubicación -->
                @enderror
            </div>
            ...
            <div class="mb-4">
                ...
                @error('categories')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p> <!-- Mensaje de error para categorías -->
                @enderror
            </div>
